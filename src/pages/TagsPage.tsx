...
import { Tag, File, FolderOpen, Plus, X, Search, ArrowUp, Monitor, Wifi, Eye } from "lucide-react";
import type { TagEntry, FileEntry, SshProfile } from "../lib/types";
import { tagColor, formatBytes } from "../lib/utils";
import { TagChip } from "../components/TagChip";
import { FileViewer } from "../components/FileViewer";

// Sub-component so it can use useLayoutEffect with its own ref
function TagInlineEditor({ file, existingTags, onAdd, onClose }: {
  file: FileEntry;
  existingTags: TagEntry[];
  onAdd: (file: FileEntry, tag: string) => void;
  onClose: () => void;
}) {
  ...
  // useLayoutEffect fires synchronously after DOM paint — ref is guaranteed set
  useLayoutEffect(() => {
    if (inputRef.current) {
    ...
    <>
      <div className="flex items-center gap-1">
        <input
          ...
          onKeyDown={(e) => {
            if (e.key === "Enter")  { onAdd(file, input); onClose(); }
            if (e.key === "Escape") { onClose(); }
            if (e.key === "ArrowDown" && suggestions[0]) { onAdd(file, suggestions[0].tag); onClose(); }
          }}
          className="bg-[#1e1e1e] border border-[#3b82f6] rounded px-2 py-0.5 text-[11px] text-[#e5e7eb] w-28 outline-none font-mono"
        />
        <button onClick={() => { onAdd(file, input); onClose(); }}
      ...
      {pos && suggestions.length > 0 && createPortal(
        <div style={{ position: "fixed", top: pos.top, left: pos.left, zIndex: 9999 }}
          ...
          {suggestions.slice(0, 6).map(({ tag }) => (
            <button key={tag}
              onMouseDown={(e) => { e.preventDefault(); onAdd(file, tag); onClose(); }}
